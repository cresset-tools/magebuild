 ...
 * magebuild GitHub release (cresset mirror first), SHA-256-verified against
 * the `.sha256` sidecar, cached per-version, and exec'd. Mirrors `uv format`
 * and Mago's `composer/bin/mago`.
 *
 * The package ships one bin shim (`bin/magebuild`) over this launcher, which
 * stages the single `magebuild` binary out of the `magebuild-{target}` archive
 * released on the `magebuild-v<version>` tag (see dist-workspace.toml).
 *
...
{
    private const TOOL = 'magebuild';
    private const MIRROR_BASE = 'https://releases.bougie.tools/github/magebuild/releases/download';
    private const GITHUB_BASE = 'https://github.com/cresset-tools/magebuild/releases/download';
    private const USER_AGENT = 'cresset/magebuild composer installer';
    ...
    private static function ensureBinary(): string
    {
        ...
        $tag = 'magebuild-v' . MAGEBUILD_VERSION;
        $archive = 'magebuild-' . $target . (self::isWindows() ? '.zip' : '.tar.gz');
        ...
        try {
            ...
            fwrite(STDERR, self::TOOL . ': fetching ' . self::TOOL . ' ' . MAGEBUILD_VERSION . " ($target)\n");
            ...
            $staged = $extractDir . DIRECTORY_SEPARATOR . 'magebuild-' . $target . DIRECTORY_SEPARATOR . $binName;
            if (!is_file($staged)) {
    ...
    private static function targetTriple(): string
    {
        ...
        switch (PHP_OS_FAMILY) {
            case 'Linux':
                ...
                if ($isArm) {
                    // dist builds only the gnu arm64 Linux target (no musl arm64).
                    return 'aarch64-unknown-linux-gnu';
                }
                throw new RuntimeException('no prebuilt ' . self::TOOL . " binary for Linux/$machine (x86_64 or arm64)");
            case 'Darwin':
                if ($isArm) {
                ...
                throw new RuntimeException('no prebuilt ' . self::TOOL . " binary for macOS/$machine");
            case 'Windows':
                if ($isX64) {
    ...
    private static function cacheDir(): string
    {
        ...
        return $base . DIRECTORY_SEPARATOR . self::TOOL . DIRECTORY_SEPARATOR . MAGEBUILD_VERSION;
    }
    ...
    {
        $dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'magebuild-dl-' . bin2hex(random_bytes(6));
